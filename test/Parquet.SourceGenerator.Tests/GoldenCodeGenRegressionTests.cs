...
{
    private static readonly string GoldenFilePath = IOPath.Combine(
        AppDomain.CurrentDomain.BaseDirectory,
        "..", "..", "..", "GoldenFiles", "OrderEventParquetExtensions.g.cs");

    [Fact]
    public void EmitsConsistentGoldenSourceForComprehensiveModel()
    {
        string emittedSource = GenerateOrderEventSource();

        // If environment variable UPDATE_GOLDEN_FILES is true or file doesn't exist, update it
        ...
        if (updateGolden || !IOFile.Exists(GoldenFilePath))
        {
            IODirectory.CreateDirectory(IOPath.GetDirectoryName(GoldenFilePath)!);
            IOFile.WriteAllText(GoldenFilePath, normalizedEmitted + "\n");
        }

        string expectedSource = IOFile.ReadAllText(GoldenFilePath).Replace("\r\n", "\n").TrimEnd();
    ...
    public static string GenerateOrderEventSource()
    {
        var properties = new[]
        {
        ...
        return CodeEmitter.EmitSource(model);
    }
