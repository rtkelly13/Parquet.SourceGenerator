...
//   docs/internals/callgraph.md               type-level Mermaid, refreshed by the same command
//   docs/internals/callgraph-generated.md     the generated code's graph, as documentation
//
// What is gated:
...
//      no allowlist kind.
//   3. Fan-out: CA-fanout ratchet, thresholds only ever come down (docs/quality/code-metrics.md policy).
...
Directory.CreateDirectory(Path.Combine(docsDir, "internals"));
...
string docsPath = Path.Combine(docsDir, "internals", "callgraph.md");
string docs =
    ...
    + "see docs/quality/call-graph.md). Full method-level edges: `graph/Parquet.SourceGenerator.callgraph.txt`.\n\n"
    + "```mermaid\n"
    + "graph TD\n"
    + mermaidTypes
    + "```\n";
...
static void WriteGeneratedGraph(string repoRoot, string docsDir, bool update)
{
    ...
    string path = Path.Combine(docsDir, "internals", "callgraph-generated.md");
