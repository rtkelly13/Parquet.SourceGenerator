---
description: "Metrics for emitted code, and why method size is watched rather than fixed."
order: 80
---
...
> The hand-written code under `src/` is [Code Metrics & the Complexity Ratchet](./code-metrics.md) (layer 1); duplication (layer 3)
...
`*.api.txt` ([Generated API Baselines](./api-baselines.md)) made the emitted *surface* visible; this makes
...
test/Parquet.SourceGenerator.Tests/GoldenFiles/
  ...
  OrderEventParquetExtensions.api.txt       <- its public surface  (docs/quality/api-baselines.md)
...
grammar for both artefacts. See [Code Metrics & the Complexity Ratchet](./code-metrics.md#a-normalised-rendering-not-the-tools-xml)
...
- **The denominator is the `.api.txt` member count**, read from the file rather than recomputed.
  ...
  API change contract ([API Change Contract](./api-change-contract.md)), and #244 established that this
...
same posture [Code Metrics & the Complexity Ratchet](./code-metrics.md) takes towards the Maintainability Index.
...
[Code Metrics & the Complexity Ratchet](./code-metrics.md) already notes that MI is a poor absolute judgement of quality and keeps
...
argument ([Performance Findings](../internals/performance-findings.md)). Gating MI here would gate the design.
...
The same contract as [Generated API Baselines](./api-baselines.md) and [Code Metrics & the Complexity Ratchet](./code-metrics.md), and the
...
`TargetParser.CollectMembers`, which [Code Metrics & the Complexity Ratchet](./code-metrics.md) calls the alarming finding of layer
...
[Performance Findings](../internals/performance-findings.md), not on the number alone.
...
([IL Interrogation](./il-interrogation.md)), which asks whether the emitted IL *boxes*; this asks how *big* it
is.
...
- **No absolute threshold, and no analyzer rules.** `CA1502`/`CA1505`/`CA1506` are scoped to
  `[src/**.cs]` ([Code Metrics & the Complexity Ratchet](./code-metrics.md#the-analyzer-rules)) and stay there. Enabling them on
...
- **Not a governed API surface.** [API Change Contract](./api-change-contract.md) governs `*.api.txt`,
...
- [Generated API Baselines](./api-baselines.md) — the `.api.txt`
  companion, and the source of the `MEMBERS` count.
- [Code Metrics & the Complexity Ratchet](./code-metrics.md) — layer 1, the
  hand-written half, and the shared format rationale.
- [Performance Findings](../internals/performance-findings.md) — why the
