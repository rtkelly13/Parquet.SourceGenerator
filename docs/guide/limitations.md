---
description: "What does not work today, and the closed audit that shaped the generator."
order: 60
---
...
deferred features and their gates live in [DECISIONS](../design/decisions.md).
...
| Compound shapes | Modern backend: tested struct/list shapes only. Classic backend: flat only. Maps unsupported. See [DECISIONS #176](../design/decisions.md#176--nested-types-by-backend). |
...
| Native AOT | CI publishes and runs the AOT binary on `linux-x64` only; the AOT matrix covers flat models. Parquet.Net itself still emits trim/AOT warnings — see [Native AOT](./native-aot.md). |
