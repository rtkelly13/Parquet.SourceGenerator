---
description: "Why the flat read methods were removed, measured shrinkage and the migration table."
order: 40
---

# Flat-Read Removal (#480)

> **Status:** decided and implemented. **Supersedes** the #262 flat-read freeze
> (closed by PR #344; summarised in [DECISIONS #262](./decisions.md#262--flat-read-freeze-superseded)).
...
the declared-subset policy of #246 ([Compatibility Matrix](../reference/compatibility.md#backend-api-policy-for-01))
...
## Why the #262 freeze no longer holds
...
- The builder terminals delegated to the flat methods, so the method bodies stay. They are now
  ...
  [Generated API Baselines](../quality/api-baselines.md#the-grammar) rule 5 neither is part of the emitted
...
- The two **`Stream` "parallel" overloads are deleted outright**, body included. They read row groups
  ...
  [Public API Surface](../reference/api-surface.md)), nothing in the builder delegated to them, and the builder
...
([Public API Surface](../reference/api-surface.md) decision D5):
