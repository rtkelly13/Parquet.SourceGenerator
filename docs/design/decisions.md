---
description: "Every scoping and deferral decision, and what must hold before it is revisited."
order: 30
---
...
[0.1 Contract & Design Goals (#477)](./release-contract.md).
...
| [#480](#480--flat-read-removal-supersedes-262) | Flat-read removal | Implemented — see [Flat-Read Removal (#480)](./flat-read-removal.md) |
...
[Unified Pushdown (Proposal)](./unified-pushdown.md) is kept as the future design reference and is
...
prerequisite for the Arrow bridge. Background: [Nested Types: Level Encoding](../internals/nested-types.md).
...
[Flat-Read Removal (#480)](./flat-read-removal.md).
...
guidance is published. #480 answered that gate — see [Flat-Read Removal § Why the #262 freeze no longer holds](./flat-read-removal.md#why-the-262-freeze-no-longer-holds).
...
row-group metadata is externally read-only ([0.1 Contract & Design Goals (#477) §4.3](./release-contract.md)).
...
(see [Attributes & Configuration § Feature levels](../guide/attributes.md#4-feature-levels)); the profile matrix
...
[Compatibility Matrix § Apache Arrow RecordBatch Ingestion](../reference/compatibility.md). Export (#178) uses the same
...
confidence/minimal-contract gate in [0.1 Contract & Design Goals (#477) §8](./release-contract.md#8-release-gate)
