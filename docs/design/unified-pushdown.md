---
description: "Future pushdown design; superseded for the current release by the generated/shipped boundary decision."
order: 50
---

# Unified Pushdown (Proposal)
...
>
> Numbered 20, not 18: `docs/quality/api-baselines.md` and `docs/quality/api-change-contract.md` were taken by #235 and #236 while this was being
...
- `docs/internals/buffer-reuse.md` §6.2 — *"Roughly 92% of a Parquet write is not the extraction. Encoding, compression and
  page assembly inside Parquet.Net dominate."*
- `docs/internals/performance-findings.md` §6.3 — *"a 6× win in extraction shows up as a few percent of wall clock."*
...
Allocation is the benchmark gate; wall-clock is reported and not enforced. `docs/internals/buffer-reuse.md` §6.3 established
...
3. Filters, planner and `.Explain()` ship; `.Where(Func<...>)` and the ordered-lookup families are
   retained for one release, then removed at the freeze, per docs/reference/api-surface.md decision D3.
...
- #235 / `docs/quality/api-baselines.md` — the signature baselines that made this surface measurable in the first place
- #236 / `docs/quality/api-change-contract.md` — the API change contract; every type specified here enters through its ledger
- #216 / `docs/reference/api-surface.md` — the surface audit this continues
