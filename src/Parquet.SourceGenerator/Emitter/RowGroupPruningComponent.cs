...
internal static class RowGroupPruningComponent
{
    ...
    )
    {
        ...
        builder.AppendLine("                {");
        builder.AppendLine("                    throw new global::System.IO.InvalidDataException($\"Row group {r} row count {rc} is invalid or exceeds maximum allowed {options.MaxAllocationValues}.\");");
        builder.AppendLine("                }");
        builder.AppendLine("                sumRows = checked(sumRows + rc);");
        builder.AppendLine("            }");
        builder.AppendLine("            if (sumRows > options.MaxAllocationValues)");
        builder.AppendLine("            {");
        builder.AppendLine("                throw new global::System.IO.InvalidDataException($\"Total row count {sumRows} exceeds maximum allowed {options.MaxAllocationValues}.\");");
        builder.AppendLine("            }");
        builder.AppendLine($"            {totalRowsVar} = checked((int)sumRows);");
        builder.AppendLine("        }");
        builder.AppendLine("        else");
        builder.AppendLine("        {");
        builder.AppendLine("            // Zone-map pre-pass: footer statistics only. Surviving groups are the only ones");
        builder.AppendLine("            // whose pages are ever read, and the result is sized to exactly their rows.");
        ...
        builder.AppendLine("            if (sumRows > options.MaxAllocationValues)");
        builder.AppendLine("            {");
        builder.AppendLine("                throw new global::System.IO.InvalidDataException($\"Total matching row count {sumRows} exceeds maximum allowed {options.MaxAllocationValues}.\");");
        builder.AppendLine("            }");
        builder.AppendLine($"            {totalRowsVar} = checked((int)sumRows);");
        builder.AppendLine("        }");
    }
