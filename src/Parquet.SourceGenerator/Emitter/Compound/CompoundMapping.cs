...
internal static class CompoundMapping
{
    ...
    private static void EmitListElementField(StringBuilder builder, LeafColumn col, string p)
    {
        int n = col.Slot;
        string cv = $"cv_{n}";
        ...
        if (col.Leaf.IsNullable)
        {
            ...
            builder.AppendLine($"{p}    defLevels_{n}[posCount_{n}] = {col.MaxDef - 1};");
            builder.AppendLine($"{p}}}");
        }
        else
        {
    ...
    )
    {
        ...
        {
            // List<POCO>: element slot gets its own rung (def == ListElementRung means a
            // null element); below that the field ladder walks with bases offset by two.
            builder.AppendLine($"{prefix}        if (el_{n} is not null) {{");
            EmitListElementField(builder, col, prefix + "            ");
            builder.AppendLine($"{prefix}        }}");
            builder.AppendLine($"{prefix}        else {{");
            builder.AppendLine(
    ...
    )
    {
        ...
        builder.AppendLine(
            $"{indent}        if (dv_{n} == {col.MaxDef}) {{ if (vc_{n} >= buffer_{n}.Length) throw new global::System.IO.InvalidDataException($\"Definition levels in column '{col.Leaf.Name}' exceeded values buffer length ({buffer_{n}.Length}).\"); bk_{n}.Add({GetCompoundLeafReadExpression(col.Leaf, $"buffer_{n}[vc_{n}++]")}); }}"
        );
        if (col.Leaf.IsNullable)
    ...
    )
    {
        ...
        {
            builder.AppendLine(
                $"{indent}            if (defLevels_{g.Slot}[p_{a}] >= {g.MaxDef} && vc_{g.Slot} >= buffer_{g.Slot}.Length) throw new global::System.IO.InvalidDataException($\"Definition levels in column '{g.Leaf.Name}' exceeded values buffer length ({buffer_{g.Slot}.Length}).\");"
            );
        }
