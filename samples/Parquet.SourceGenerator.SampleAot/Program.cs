...
internal static class Program
{
    private static async Task Main()
    {
        ...
        Console.WriteLine($"[1/2] Serializing {transactions.Count} transactions to Parquet stream (Native AOT safe)...");
        ...
        List<FinancialTransaction> deserialized = await FinancialTransactionParquetExtensions.ReadParquetAsync(memoryStream);

        Console.WriteLine($"✅ Successfully deserialized {deserialized.Count} records!");
        Console.WriteLine($"   Record 1 Account: {deserialized[0].AccountNumber}, Amount: ${deserialized[0].Amount}");
        Console.WriteLine($"   Record 2 Account: {deserialized[1].AccountNumber}, Amount: ${deserialized[1].Amount}");
        Console.WriteLine("=================================================");
